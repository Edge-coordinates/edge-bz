<script>
    ...
    onMount(async () => {
        ...
        function init() {
            ...
            const flowerTexture = new THREE.TextureLoader().load('/flower.png')
            ...
            if (stringBox.wTexture > 0) {
                ...
                for (let i = 0; i < textCanvas.height; i++) {
                    for (let j = 0; j < textCanvas.width; j++) {
                        imageMask[i][j] =
                            imageData.data[(j + i * textCanvas.width) * 4] > 0
                    }
                }
                ...
                if (textureCoordinates.length !== 0) {
                    ...
                    textureCoordinates = textureCoordinates.filter(
                        (c) => !c.toDelete
                    )
        ...
        function recreateInstancedMesh() {
            ...
            const totalNumberOfFlowers = particles.filter(
                (v) => v.type === 0
            ).length
            const totalNumberOfLeafs = particles.filter(
                (v) => v.type === 1
            ).length
            ...
            particles.forEach((p) => {
                if (p.type === 0) {
                    flowerInstancedMesh.setColorAt(
                        flowerIdx,
                        new THREE.Color('hsl(' + p.color + ', 100%, 50%)')
                    )
                    flowerIdx++
                } else {
                    leafInstancedMesh.setColorAt(
                        leafIdx,
                        new THREE.Color('hsl(' + p.color + ', 100%, 20%)')
                    )
                    leafIdx++
                }
            })

            leafInstancedMesh.position.x = flowerInstancedMesh.position.x =
                -0.5 * stringBox.wScene
            leafInstancedMesh.position.y = flowerInstancedMesh.position.y =
                -0.5 * stringBox.hScene
        }
        ...
        function makeTextFitScreen() {
            ...
            const dx = Math.abs(
                (0.55 * stringBox.wScene) / Math.tan(0.5 * fovH)
            )
        ...
        function updateCursorPosition() {
            cursorMesh.position.x =
                -0.5 * stringBox.wScene + stringBox.caretPosScene[0]
            cursorMesh.position.y =
                0.5 * stringBox.hScene - stringBox.caretPosScene[1]
        }
        ...
        function updateCursorOpacity() {
            ...
            if (document.hasFocus() && document.activeElement === textInputEl) {
                cursorMesh.material.opacity = roundPulse(
...
<div class="the-flower-text">
    <div id="text-input" contenteditable="false" />
    <div class="container" />
</div>
